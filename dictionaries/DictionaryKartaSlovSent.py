...
class DictionaryKartaSlovSent:
    ...
    def get_word_tag(self, word) -> str:
        """
        Определяет тональность слова по тональному словарю. Если слова нет в словаре, тональность считается нейтральной.
        ...
        word_info = self._dictionary.loc[self._dictionary['term'] == word.lemma_].values
        if word_info.size > 0:
